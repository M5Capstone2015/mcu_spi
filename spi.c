...
//    break;
  ...
  spi->CTRL = USART_CTRL_SYNC | USART_CTRL_MSBF | USART_CTRL_BYTESWAP;
  /* Clearing old transfers/receptions, and disabling interrupts, set to shift register to msb first, swap byte order */
  spi->CMD = USART_CMD_CLEARRX | USART_CMD_CLEARTX;
  spi->IEN = 0;
  /* Set USART Frame to 16bits*/
